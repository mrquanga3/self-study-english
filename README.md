...
| Provider | Cost | Used for | Notes |
|---|---|---|---|
| **Microsoft Edge** (`msedge-tts`) | **Free**, no key | All pronunciation sets + the 18 accents | Can't clone or do expressive/shouting styles; the free endpoint ignores `<phoneme>` / styles |
| **ElevenLabs** | **Paid** (API key) | Expressive *game* voices in `audio/game/` | Free tier only allows the **default** premade voices via API; **Voice-Library voices and voice cloning require a paid plan** |

### Game voices (ElevenLabs — paid)

Expressive, game-style voices live under `audio/game/`:

```
audio/game/
  tactical/       FPS combat callouts (clean)
  tactical-fx/    same, with walkie-talkie radio FX
  halflife-fx/    Half-Life lines, radio FX
  voice-options/  A/B samples of several free ElevenLabs voices
```

Generated by `elevenlabs-tactical.js` / `elevenlabs-halflife.js`. Setup:

1. Get a key at <https://elevenlabs.io/app/settings/api-keys>.
2. `copy .env.example .env` and paste the key (`.env` is git-ignored).
3. `node elevenlabs-tactical.js` (set `ELEVENLABS_VOICE_ID` to choose a voice).

**Limits:** the free tier can use the built-in premade voices only. Using a
Voice-Library voice (e.g. "Vector – Tec Voice") or cloning a voice from an audio
